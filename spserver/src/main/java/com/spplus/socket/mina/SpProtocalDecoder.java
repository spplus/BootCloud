...
public class SpProtocalDecoder implements ProtocolDecoder {  
    private final AttributeKey CONTEXT = new AttributeKey(getClass(), "context");  
    private final Charset charset;  
    private int maxPackLength = 1000;  
    public SpProtocalDecoder() {  
        this(Charset.defaultCharset());  
    }  
    public SpProtocalDecoder(Charset charset) {  
    ...
    public void decode(IoSession session, IoBuffer in, ProtocolDecoderOutput out) throws Exception {  
        ...
        while (buf.remaining() >= packHeadLength) {  
            ...
            int length = Integer.reverseBytes(buf.getInt());  
            
            pos = buf.position();
            
            short head = Short.reverseBytes(buf.getShort());
            pos = buf.position();
            ...
            //读取正常的消息包，并写入输出流中，以便IoHandler进行处理  
            	...
            	int msgtype = Integer.reverseBytes(buf.getInt());
                pos = buf.position();
            	
                //buf.limit(buf.position() + length - packHeadLength); 
                int contentLength = length-2-4-2;
                byte[] data = new byte[contentLength+100];
                String content = "";
                if(buf.remaining()>=contentLength){
                	
                	buf.get(data, buf.position(), contentLength);
                    pos = buf.position();
                    content = new String(data);
                
                }
                
                tail = Short.reverseBytes(buf.getShort());
                pos = buf.position();
                
                buf.limit(oldLimit2);  
                SpMessage pack = new SpMessage(length,head,msgtype,tail, content);  
                out.write(pack);  
            } else {  
                // 如果消息包不完整  
                // 将指针重新移动消息头的起始位置   
